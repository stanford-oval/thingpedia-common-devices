...
const URL = "https://api.instagram.com/v1/tags/%s/media/recent?access_token=%s&count=%d";

module.exports = new Tp.ChannelClass({
    Name: 'InstagramByTagChannel',
    ...
    invokeQuery(filters) {
        var tagName = filters[0];
        var count = filters[1];
        if (count === undefined || count === null)
            count = 5;

        var url = URL.format(encodeURIComponent(tagName), this.device.accessToken, count);
        console.log('url', url);
        ...
        return Tp.Helpers.Http.get(url).then((response) => {
            ...
            return parsed.data.map((obj) => {
                return [tagName, count, obj.user.username, obj.id, obj.images.standard_resolution.url, obj.caption.text, obj.link];
            });
        });
    },
});
