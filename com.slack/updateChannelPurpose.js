...
/**
 * Updates the specified Slack channel's purpose.
 */
function setChannelPurpose(token, channel_id, purpose) {
  // Construct the proper JSON message and send to channel
  Tp.Helpers.Http.post('https://slack.com/api/channels.setPurpose',
      'token=' + token +
...
module.exports = new Tp.ChannelClass({
  Name: 'SlackUpdateChannelPurposeChannel',

  _init: function(engine, device) {
    this.parent();
    console.log('[info] Initializing SLACK:UpdateChannelPurpose');
  ...
  sendEvent: function(event) {
    ...
    Tp.Helpers.Http.post('https://slack.com/api/channels.list',
      ...
      .then(function(response) {
        ...
        setChannelPurpose(token, channel_id, event[1]);
