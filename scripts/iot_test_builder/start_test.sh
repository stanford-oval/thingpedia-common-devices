...
if [[ $1 == "P" || $1 == "p" ]]; then 
node $directory/init_test_unit.js $@ &
exit
fi
echo -e '\r'
...
echo -e '\r'
echo "Do you want to go in (S)tandard or (I)nteractive mode?"
read argS
echo -e '\r'
    if [[ $argS == "S" || $argS == "s" ]]; then 
        echo -e '\r'
        node $directory/init_test_unit.js -M
        echo "Please provide the full command as in the manual (WITHOUT \"node /path/script-name\" at the beginning)"
        read values
        node $directory/init_test_unit.js $values
