...
# These are in-domain: ok|contextual command|ambiguous|redacted|meta-command|chatty
# We ignore these: bug-tokenize
# out of domain:
sed -E -e '/\t(phone|foreign language|dialogue-model|gnome|math|new device|new function|policy or remote program|question|skill-help|web search|unintellegible|stream|tutorial)$/!d' -e 's/\t/\tnull\t/' -e 's/\t[^\t]*$/\t\$ood ;/' dropped-log.tsv > calibration-ood.tsv
# junk
